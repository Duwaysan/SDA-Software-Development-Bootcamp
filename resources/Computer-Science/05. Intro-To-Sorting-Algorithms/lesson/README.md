# ![Computer Science - Intro to Sorting Algorithms - Lesson](./assets/hero.png)

## What is Sorting?

<a href="https://generalassembly.wistia.com/medias/2ylrs7tfy7?wvideo=2ylrs7tfy7"><img src="https://embed-ssl.wistia.com/deliveries/1c92ada407c1842e1115405c459725e055ae5e73.jpg?image_crop_resized=900x506&image_play_button=true&image_play_button_size=2x&image_play_button_color=222222e0" alt=" Intro to Sorting" width="450" height="253" /></a>
...
If the only thing we’re trying to accomplish is putting things in order, how many sorting algorithms could there be? Surely it can’t be that complicated! You’d be surprised. There are dozens of sorting algorithms! There’s bubble sort, merge sort, quick sort, heap sort, radix sort, cube sort, bucket sort, shell sort, tree sort, insertion sort, selection sort, counting sort — and those are just the more common ones!
...
![Comparison Sort](https://ga-instruction.s3.amazonaws.com/assets/tech/computer-science/intro-sorting/comparison.gif)
...
![Distribution Sort](https://ga-instruction.s3.amazonaws.com/assets/tech/computer-science/intro-sorting/distribution.gif)
...
![Clothing Sorting](assets/originals/2-Sorting-Clothes.png)
...
In this instance, you’re grouping items into categories of similar things, rather than comparing each individual item to the next. That’s a distribution sort. If you were to sort all your t-shirts from oldest to newest, you might use a comparison sort.
...
<a href="https://generalassembly.wistia.com/medias/116t6c4vsc?wvideo=116t6c4vsc"><img src="https://embed-ssl.wistia.com/deliveries/cccdbe7f3cf054a7cb7866e116dfefd0691fd868.jpg?image_crop_resized=900x506&image_play_button=true&image_play_button_size=2x&image_play_button_color=222222e0" alt=" Intro to Sorting- Video 2" width="450" height="253" /></a>
...
When we sorted the second time, we lost the relationship between the correct number of problems solved and time spent. Whoops! There might be some unhappy programmers after this…
...
Quadratic complexity algorithms are the least efficient. If you have a dataset with N items, you’ll perform an operation N^N times. Not good!
...
Check out how different algorithms perform relative to one other in the next section. There’s no need to watch the entire video, but check out as much as you want in order to get a feel for the different algorithm speeds.

> (Warning: There are lots of moving colors and fast speeds in the video. If that isn't your thing, check out this [simpler version](https://www.toptal.com/developers/sorting-algorithms).)

![Worst Average Best](assets/originals/1-Worst-Average-Best.png)

## Time and Space, Visualized

[![Visualization and Comparison of Sorting Algorithms](https://img.youtube.com/vi/ZZuD6iUe3Pc/0.jpg)](https://www.youtube.com/watch?v=ZZuD6iUe3Pc)
...
| Sort           | Best Case   | Average Case | Worst Case  | Space       | Stable | Type         |
| -------------- | ----------- | ------------ | ----------- | ----------- | ------ | ------------ |
| Bubble Sort    | `Ω(N)`      | `Θ(N^2)`     | `O(N^2)`    | `O(1)`      | Yes    | Comparison   |
| Insertion Sort | `Ω(N)`      | `Θ(N^2)`     | `O(N^2)`    | `O(1)`      | Yes    | Comparison   |
| Bucket Sort    | `Ω(N+K)`    | `Θ(N+K)`     | `O(N^2)`    | `O(N+K)`    | Yes    | Distribution |
| Radix Sort     | `Ω(NK)`     | `Θ(NK)`      | `O(NK)`     | `O(N+K)`    | Yes    | Distribution |
| Merge Sort     | `Ω(log(N))` | `Θ(log(N))`  | `O(log(N))` | `O(N)`      | Yes    | Comparison   |
| Quick Sort     | `Ω(log(N))` | `Θ(log(N))`  | `O(N^2)`    | `O(log(N))` | No     | Comparison   |
