...
### SETUP
You will need to setup this exercise by installing the prompt-sync package like we did yesterday morning for the peanut butter and jelly exercise. You will also need to create a `gladiator.js` file to write the code and use `node gladiator` to run the file.
...
🔴       Be sure to commit upon completion of _each_ part (including the bonus). 
...
- Before a losing gladiator is eliminated, the user should be prompted to put
  ...
  removed. (Life isn't fair). <br>**Hint:** look up the `prompt()` method.
