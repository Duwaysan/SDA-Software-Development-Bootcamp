...
function getRandomDogHandler() {
	fetch(randomDogUrl)
		.then((response) => { dogImageTag.setAttribute('src', response.data.message) })
		.catch(error => console.log(error))
		.finally(() => console.log('finally!'));
}

function listAllBreeds() {
	fetch(listAllBreedsUrl)
		.then((response) => {
...
function createSelectTags(dogBreeds) {
	...
	dogsSelectTag.addEventListener('change', (event) => {
		...
		const dogChoiceURL = `https://dog.ceo/api/breed/${dogChoice}/images`
		return fetch(dogChoiceURL)
			.then(response => {
